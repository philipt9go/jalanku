...
export default function ResultCalculator({ initial, destination }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── SHARE ───────────────────────────────────── */}
      <a
        href={`https://api.whatsapp.com/send?text=I spend RM ${Math.round(car.totalMonthly)}/month just to commute (${initial.route.originLabel} → ${initial.route.destinationLabel}).%0A%0ABest option: ${winner.label} at RM ${Math.round(winner.cost)}/month${monthlySaving > 0 ? ` — saving RM ${monthlySaving}/month or RM ${Math.round(annualSaving).toLocaleString()}/year` : ''}.%0A%0ACheck yours: https://jalanku.my?utm_source=whatsapp&utm_medium=share`}
        target="_blank"
